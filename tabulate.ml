...
let tabulate filename = 
  In_channel.with_file filename ~f:(fun chan ->
    let header_result = In_channel.input_line chan |> Result.of_option ~error:"Unable to parse header row" in
    let prepare_table_result = Result.map header_result ~f:(fun header_line ->
      let header = String.split ~on:'\t' header_line in 
    ...
    match prepare_table_result with 
      | Result.Ok (column_hints, rows) -> draw_table column_hints rows
